...
function Latex({ latex }) {
  ...
  return <span ref={ref} style={{ fontFamily: T.code.fontFamily, color: C.text }} />;
}
...
export default function ResultsPanel({ session, equations, results, backend, escalating, escalateError, onEscalate }) {
  ...
  return (
    <div style={{ fontFamily: T.code.fontFamily, marginTop: 14 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <span style={{ fontSize: T.caption.fontSize, color: C.teal, letterSpacing: "0.22em", textTransform: "uppercase" }}>Results</span>
        {backend === "sympy" && (
          <span style={{ fontSize: T.caption.fontSize, color: C.blue, border: `1px solid ${C.blue}`, borderRadius: 3, padding: "1px 4px" }}>SymPy</span>
        )}
        {canEscalate && (
      ...
      {objectives.map((eq, i) => {
        ...
        return (
          <div key={eq.id} style={{ borderLeft: `3px solid ${accent}`, paddingLeft: 10, marginBottom: 12 }}>
            <div style={{ fontSize: T.code.fontSize, color: accent, marginBottom: 4 }}>{eq.label}</div>
            {ops.map((op) => {
              const res = r[op];
              if (!res) return null;
              ...
              return (
                <div key={op} style={{ marginBottom: 8 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                    <span style={{ fontSize: T.caption.fontSize, color: C.text, fontWeight: 600, minWidth: 56 }}>{OP_LABEL[op] || op}</span>
                    {fallback && (
                      <span style={{ fontSize: T.caption.fontSize, color: C.gold, border: `1px solid ${C.gold}`, borderRadius: 3, padding: "1px 4px" }}>
                        numeric-fallback
                      </span>
                    )}
                    {res.symbolic?.latex && (
                      <button onClick={() => copy(res.symbolic.latex)}
                        style={{ fontSize: T.caption.fontSize, marginLeft: "auto", background: C.surface2, color: C.textDim,
                  ...
                  {res.symbolic?.latex && (
                    <div style={{ fontSize: T.body.fontSize, color: C.text, marginBottom: 3, overflowX: "auto" }}>
...
function NumericReadout({ op, numeric, C }) {
  ...
  if (op === "integral") {
    ...
    return <span style={{ fontSize: T.code.fontSize, color: dim }}>∫ over [{numeric.a}, {numeric.b}] = <b style={{ color: C.text }}>{fmt(numeric.value)}</b></span>;
  }
  if (op === "solveZero") return <span style={{ fontSize: T.code.fontSize, color: dim }}>roots: <b style={{ color: C.text }}>{(numeric.roots || []).map(fmt).join(", ") || "none in view"}</b></span>;
  if (op === "optimize") {
    ...
    return (
      <div style={{ fontSize: T.code.fontSize, color: dim }}>
