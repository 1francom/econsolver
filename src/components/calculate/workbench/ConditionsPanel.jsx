  ...
  return (
    <div style={{ fontFamily: mono, marginTop: 14, borderTop: `1px solid ${C.border2}`, paddingTop: 10 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        <div style={{ fontSize: 9, color: C.gold, letterSpacing: "0.22em", textTransform: "uppercase" }}>
          Conditions
        </div>
        <button onClick={() => onAdd()} title="Add an intersection / first-order condition"
          style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, cursor: "pointer",
            background: "transparent", color: C.gold, border: `1px solid ${C.gold}`, fontFamily: mono }}>
          + condition
        </button>
        {names?.length > 0 && (
          <span style={{ fontSize: 9, color: C.textDim || "#888" }}>names: {names.join(", ")}</span>
        )}
      </div>

      {conds.length === 0 && (
        <div style={{ fontSize: 10, color: C.textDim || "#888" }}>
        ...
        return (
          <div key={c.id} style={{ marginTop: 8, opacity: dim ? 0.5 : 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
              ...
              <span style={{ fontSize: 10, color: C.textDim }}>w.r.t</span>
              ...
              <button onClick={() => onRemove(c.id)} title="remove"
                style={{ marginLeft: "auto", fontSize: 12, lineHeight: 1, padding: "1px 6px", cursor: "pointer",
                  background: "transparent", color: C.textDim, border: `1px solid ${C.border2}`, borderRadius: 4, fontFamily: mono }}>
                ×
              </button>
            </div>

            {c.enabled !== false && res && (
              <div style={{ fontSize: 11, marginTop: 3, marginLeft: 22 }}>
                {res.error ? (
