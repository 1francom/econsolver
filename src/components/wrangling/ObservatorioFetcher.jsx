  ...
  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:400, background:"rgba(0,0,0,0.82)", display:"flex", alignItems:"center", justifyContent:"center" }}>
      <div onClick={e => e.stopPropagation()} style={{ width:"min(620px,96vw)", background:C.surface, border:`1px solid ${C.border2}`, borderRadius:6, display:"flex", flexDirection:"column", overflow:"hidden", boxShadow:"0 20px 60px rgba(0,0,0,0.7)" }}>
        ...
        <div style={{ padding:"1rem 1.1rem", fontSize: T.code.fontSize, color:C.textDim, fontFamily: T.code.fontFamily, lineHeight:1.6 }}>
          ...
          <div style={{ marginTop:8, color:C.textMuted, fontSize: T.caption.fontSize }}>
        ...
        <div style={{ padding:"0 1.1rem 0.6rem" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5 }}>
            <div style={{ flex:1, fontSize: T.caption.fontSize, color:C.textMuted, letterSpacing:"0.18em", textTransform:"uppercase", fontFamily: T.code.fontFamily }}>1 · Pull JSON from your logged-in tab</div>
            <button onClick={copySnippet} style={{ background:"transparent", border:`1px solid ${C.border2}`, borderRadius:3, color: copied?C.teal:C.textDim, cursor:"pointer", fontFamily: T.code.fontFamily, fontSize: T.caption.fontSize, padding:"0.2rem 0.55rem" }}>{copied ? "✓ Copied" : "⧉ Copy console snippet"}</button>
          </div>
          <div style={{ fontSize: T.caption.fontSize, color:C.textMuted, fontFamily: T.code.fontFamily, lineHeight:1.55, marginBottom:10 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:5 }}>
            <div style={{ flex:1, fontSize: T.caption.fontSize, color:C.textMuted, letterSpacing:"0.18em", textTransform:"uppercase", fontFamily: T.code.fontFamily }}>2 · Paste or upload</div>
            <label style={{ background:"transparent", border:`1px solid ${C.border2}`, borderRadius:3, color:C.textDim, cursor:"pointer", fontFamily: T.code.fontFamily, fontSize: T.caption.fontSize, padding:"0.2rem 0.55rem" }}>
              ↑ Load .json
          ...
          <textarea
            ...
            spellCheck={false}
            style={{ width:"100%", boxSizing:"border-box", height:140, resize:"vertical", background:C.bg, border:`1px solid ${C.border2}`, borderRadius:4, color:C.text, fontFamily: T.code.fontFamily, fontSize: T.code.fontSize, padding:"0.55rem 0.65rem", lineHeight:1.5 }}
          />
        </div>

        {error && (
          <div style={{ margin:"0 1.1rem 0.9rem", padding:"0.6rem 0.75rem", background:`${C.red}15`, borderLeft:`3px solid ${C.red}`, fontSize: T.code.fontSize, color:C.red, fontFamily: T.code.fontFamily }}>⚠ {error}</div>
        )}
        {meta && (
          <div style={{ margin:"0 1.1rem 0.9rem", fontSize: T.caption.fontSize, color:C.textMuted, fontFamily: T.code.fontFamily }}>
        ...
        <div style={{ padding:"0.65rem 1.1rem", background:C.bg, borderTop:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:10 }}>
          <div style={{ flex:1, fontSize: T.caption.fontSize, color:C.textMuted, fontFamily: T.code.fontFamily }}>Source: observatorioluciaperez.org · public registry</div>
          ...
          <button onClick={handleImport} disabled={!text.trim()}
            style={{ padding:"0.38rem 1.1rem", background:C.gold, border:"none", borderRadius:3, color:C.bg, cursor: text.trim()?"pointer":"not-allowed", fontFamily: T.code.fontFamily, fontSize: T.code.fontSize, fontWeight:700, opacity: text.trim()?1:0.5 }}>
            ↓ Import registry
          </button>
        </div>
      </div>
