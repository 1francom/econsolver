...
export function generateWorkspaceScript({ language, datasets, globalPipeline = [] }) {
  ...
  if (language === "r") {
    ...
    for (const ds of ordered) {
      ...
      lines.push(`# ${"─".repeat(60)}`);
      if (isInAppDataset(ds, globalPipeline)) {
        lines.push(`# ${df} is built from its source datasets in the Cross-dataset section below — not loaded from a file.`);
      } else {
  ...
  if (language === "stata") {
    ...
    for (const ds of ordered) {
      ...
      lines.push(`* ${"─".repeat(60)}`);
      if (isInAppDataset(ds, globalPipeline)) {
        lines.push(`* ${ds.name} is built from its source datasets in the Cross-dataset section below — not loaded from a file.`);
        lines.push(``);
        continue;
      }
