...
// ─── PROMPT: SCRIPT NOTES (commentary for the deterministic unified script) ──
// Called by generateScriptNotes() in AIService.js. The unified replication
// script is now built deterministically (services/export/unifiedScript.js) —
// the model NEVER rewrites code: it had rewritten data prep and dropped a
// filter (LMU PS5). It only writes a comment block that goes above the code.
export const SCRIPT_NOTES_PROMPT = `\
────────────────────────────────────────────────────────────────────
TASK: REPLICATION SCRIPT NOTES
────────────────────────────────────────────────────────────────────
You receive a complete, already-verified replication script (R, Stata or
Python) generated by Litux. DO NOT rewrite, fix, reorder or repeat any code.

Write a short header of COMMENTS ONLY that a referee or a student would want
above this script:
  1. What the analysis does, in 2-4 lines (data, sample restrictions, outcome).
  2. One line per model block, in the order they appear: estimator, outcome,
     key regressor(s), fixed effects / instruments, standard errors.
  3. Identifying assumptions for the designs present (parallel trends,
     exclusion restriction, continuity at the cutoff, …) — only those present.
  4. Anything a replicator must do by hand that the script itself flags
     (files to place next to the script, NOTE lines).

Rules:
- EVERY line must start with the comment character you are told to use.
- No code, no code fences, no markdown headings, at most 40 lines.
- Only describe what is in the script. Do not invent results or numbers.
- Plain English unless told otherwise.`;
...
3.  Collapse redundant intermediate assignments: if a variable is
    assigned and immediately overwritten with NO OTHER CODE READING THE OLD
    VALUE in between, keep only the final value. Likewise, if the SAME
...
3a. NEVER apply rule 3 to a step that changes the ROW SET or GROUPING of a
    ...
    appear in the supplied sections, keep BOTH unless the first one's entire
    effect (row set AND columns) is provably a strict subset of the second's.
...
6b. DO NOT INVENT econometrics. Never add estimation code, diagnostics,
    ...
    - Emit \`#install.packages("")  # if not installed\` with the '#' — for all packages that might not be installed by
      ...
      Whenever a .docx modelsummary/stargazer call is present ANYWHERE in the
      script — active OR commented-out as the non-default option — also add
      \`#install.packages("pandoc")  # only needed if you switch back to .docx output\`
      next to the Setup install.packages list (R), or the equivalent commented
      pip/pandoc install note for Python (\`# pip install pandoc  # only needed for .docx export\`).
      Do NOT add an unconditional \`library(pandoc)\`/import — pandoc is only
      needed if the user re-enables .docx, so keep that reference commented too.
...
7.  At the end, add a brief comment block explaining the main model spec.
    This block is PROSE ONLY (comments) — never re-emit, paraphrase-as-code,
    or rebuild any chart, table, or estimation call that already appears
    earlier in the script (e.g. do not re-derive an Explore time-series chart
    a second time just to illustrate the spec note). If you want to refer to
    an earlier result, name it in words ("see the time-series chart above").
8.  HONOR THE SESSION SNAPSHOT (when provided):
    ...
    b. If "REQUIRED LOAD CALL(S)" lines are provided, those load calls are
       ...
       not use it. EXCEPTION: some entries are COMMENT lines stating a dataset
       is "produced inside Litux … regenerated by the Spatial operations
       section … do NOT load it from a file" (often followed by a commented-out
       load call). Emit BOTH of those lines byte-for-byte as given, including
       the leading comment character on EACH line — do not drop, trim, or
       convert the '#'/'*' prefix on the second (fallback) line into a plain
       leading space, even if you are reformatting whitespace elsewhere. A line
       that reads "# df_x <- read_csv(...)" must NEVER become
       " df_x <- read_csv(...)" (no '#') in your output — that silently turns
       a commented-out fallback into a live, wrong data load, because the file
       it points to does not exist and the real values are produced by the
       "── Spatial operations ──" section appended later. If you are unsure
       whether a character survived your rewrite, copy the two lines verbatim
