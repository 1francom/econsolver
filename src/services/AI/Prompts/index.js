...
8.  HONOR THE SESSION SNAPSHOT (when provided):
    ...
    b. If "REQUIRED LOAD CALL(S)" lines are provided, those load calls are
       ...
       load call). Emit those lines EXACTLY as given — keep the comment a
       comment and keep the commented load call commented. Do NOT turn them into
       an active read_csv/import: those datasets (spatial grids, spatial-join /
       aggregate results) are rebuilt by the "── Spatial operations ──" section
       appended later, so loading them from a file is wrong (the file does not
       exist).
