// ─── ECON STUDIO · services/export/rScript.js ────────────────────────────────
// Transpiler: Econ Studio pipeline + model config → R script
//
// Target packages:
...
export function generateRScript(config) {
  ...
  lines.push(
    `# ════════════════════════════════════════════════════════════════════════`,
    `# Replication script — generated by Econ Studio`,
    `# Dataset:  ${filename}`,
  ...
  if (pipeline.length) {
    ...
    lines.push(`# ${pipeline.length} step${pipeline.length !== 1 ? "s" : ""} applied in Econ Studio`);
    lines.push(``);
...
export function generateMultiModelRScript(configs = [], dataDictionary = null) {
  ...
  lines.push(
    `# ════════════════════════════════════════════════════════════════════════`,
    `# Multi-model replication script — generated by Econ Studio`,
...
export function generateSubsetRScript({ filename = "dataset.csv", pipeline = [], perSubsetSteps = [], subsets = [], model = {}, dataDictionary = null } = {}) {
  ...
  lines.push(
    `# ════════════════════════════════════════════════════════════════════════`,
    `# Multi-subset replication script — generated by Econ Studio`,
    `# Dataset:  ${filename}`,
    `# Model:    ${model.type ?? "unknown"}`,
