...
| 2026-06-01 | `specs/2026-06-01-spatial-engine-gaps-design.md` | IN PROGRESS | A4 expand_grid COVERED via `balance_panel`; Part A (areal interp, buffer exposure/count) + Part C (spatial weights, Moran/Geary, KDE, zonal, C5 spatial regression engine/UI) landed in `SpatialEngine.js` / `SpatialRegressionEngine.js` + UI sections, harnesses `spatialGapsValidation.js` and `spatialRegressionValidation.js` |
