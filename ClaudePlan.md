...
| 2026-06-05 | `specs/2026-06-05-excel-grid-editing-design.md` | IN PROGRESS (export translations pending Codex merge) | Roadmap item 2: Excel-grid editing in Data Viewer. 5 new steps (add_column, add_row, set_where, replace, str_splice) + structured filter bar as WHERE predicate. Non-destructive (emits pipeline steps). DataViewer (App.jsx) + DataStudio studio methods. Export translations sequenced after Codex Batch-1 export merge. Plan `plans/2026-06-05-excel-grid-editing.md` (6 tasks). Claude track |
