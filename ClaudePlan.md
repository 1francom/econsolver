...
|------|-------------|--------|-------|
| 2026-07-10 | `plans/2026-07-10-panel-nway-fixed-effects.md` | IN PROGRESS — Tasks 1-8 code-complete, R validation pending Franco | From Supabase feedback: users can't choose FE columns for TWFE/FE/LSDV/EventStudy beyond the single entity/time pair declared in PanelTab, and can't cross two columns into one FE interaction group. 8-task plan: (1) generalized N-way alternating-projection demeaning engine `PanelWithinEngine.js` (same Gaure/Correia method already proven in `NonLinearEngine.js`'s `runPoissonFEMulti`), (2) FE interaction column materialization, (3) `PanelTab.jsx`/`validatePanel` gain `feCols[]` (entity/time kept as `feCols[0/1]` for backward compat), (4) `PanelEngine.js` FE/FD/TWFE/EventStudy/LSDV rewritten with `*Multi` variants + legacy wrappers, (5) UI wiring (`ModelConfiguration.jsx`/`ModelingTab.jsx`/`estimationDispatch.js`), (6) DuckDB SQL fast-path explicitly capped at 2 FE dims with a fallback guard (documented scope cut, not silent), (7) R/Python/Stata replication export for N-way absorption, (8) required R validation incl. a nested-FE case that may expose a df-formula gap (flagged, not hidden). Brainstormed + scoped with Franco 2026-07-10 (chose "Full N-way FE" + "include FE interactions now"). **Task 8 status (2026-07-11):** R harness `src/math/__validation__/panelNwayFeRValidation.R` written (no R on this machine — Franco must run it and commit `panelNwayFeBenchmarks.json`/`panelNwayFeFixture.csv`); `engineValidation.js` has a defensive comparison suite (2-way independent, 3-way independent, nested state+state×year cases) that currently skips cleanly with a one-line message since the benchmark files don't exist yet. CLAUDE.md estimator table updated to show N-way FE as implemented-but-unvalidated. Feature is NOT done until Franco runs the R script and the nested-case df comparison is resolved one way or the other.|
