...
|------|-------------|--------|-------|
| 2026-09-17 | `plans/2026-09-17-real-data-validation.md` | OPEN — plan written, awaiting Franco's answers (project names, AJR data, package installs) | Real-data validation replacing the deleted simulated benchmarks: LMU Applied Econometrics PS2–PS6 + Labor market Tutorial 6 (PS1, PS7, BA thesis out of scope). Litux numbers are recomputed headlessly from an exported project recipe (new *Export project* `.litux.json`), every full and per-section script is generated from the same specs and run in R/Stata/Python, all compared against the original reference script. Starts with PS5 (Waldinger Validation Tutorial 5, already built). |
| 2026-08-28 | `plans/2026-08-28-join-semantics-and-lineage-snapshots.md` | **IMPLEMENTATION COMPLETE 2026-08-28** — all 15 tasks; browser validation pending Franco (11-point checklist at the end of the plan doc) | Join = dplyr `left_join` (the JS runner collapsed to first-match while the DuckDB path already emitted a real SQL JOIN — the same step returned a different row count depending only on dataset size); new validated m:1 `lookup` step that throws instead of silently picking a match; G-step v2 freezes each parent's `{name, filename, loadOpts, pipeline-as-of-then}` so a derived dataset stays reconstructible after its parent is edited or deleted (R value semantics); join can write its result to a new dataset (forma 1) without touching the parent's pipeline; exporter three-case emission (identical / prefix / diverged) + `topoSort` inversion fix + `isInAppDataset` off the filename heuristic + cross-dataset section appended AFTER the Report AI instead of through it (it was being dropped wholesale). All harnesses green: `joinCardinalityValidation` + `lineageValidation` (both new), `pipelineReliability` 91/0, `datasetContext` 21/0, `replicationIntegrity` 351/1 (the 1 is the pre-existing `connected_components` gap, present at session start), `lint:undef` 367 files clean, `npm run build` green. **PENDING FRANCO — Stata m:m join:** no single Stata instruction replicates dplyr's join (`merge 1:1` errors on any repeated key, `merge 1:m` on a repeated master key, `joinby` expands but has no `suffixes()`). Both emitters use `merge 1:m` + a NOTE naming `joinby` and marked UNVERIFIED; Franco verifies against a real Stata run at LMU and picks. |
| 2026-08-20 | `specs/2026-08-20-model-explore-import-export-design.md` + `plans/2026-08-20-model-explore-import-export.md` | **IMPLEMENTATION COMPLETE 2026-08-22** — build + lint:undef green, 4 harnesses green (`pipelineReliabilityValidation` 90, `modelSpecValidation` 391, `artifactIOValidation` 97, plus the artifactIO hardening pass folded into that count), browser validation pending Franco | Gives Model and Explore the round-trip Clean already has: `model.json` (pinned model **specs**) and `plots.json` (saved plot entries), one file per module, same shape/validation/abort discipline as `pipeline.json`. Model import fills the sidebar and the user presses Estimate — the recipe travels, never the coefficients, so no foreign numbers enter the model buffer. Explore import appends to `plotHistory` (no History panel to undo with, unlike the pipeline import). **The load-bearing part is a prerequisite fix, not the file format:** `result.spec` is built per-estimator inside `estimationDispatch.js` and is incomplete for several — the file admits at line 410 that Callaway-Sant'Anna params "were passed to runCallawayCS but never captured in spec" — so exporting from it would silently ship broken files for SC/PoissonFE/Sun-Abraham/CS/Spatial. New `modelSpec.js` declares one `MODEL_SPEC_FIELDS` table (each field tagged `column`/`columns`/`columnMap`/`enum`/`datasetRef`/`scalar`) driving `collectSpec`/`applySpec`, consumed by three call sites: `specExtras`, pin restore, and import. Side effect: closes the existing restore gap where `onRestore` refills only ~12 fields and skips `seType` behind a comment that stopped being true once `specExtras` started stamping it. `exprGuard` is deliberately NOT wired in — verified that plot layers and model specs carry no evaluated expression today; noted as the condition if one ever appears. **Found while specc'ing, deliberately deferred to its own spec:** derived datasets aren't just missing from the export, they're never recorded — Clean's `doSaveSubset()` (`WranglingModule.jsx:484`) calls `onSaveSubset` with no recipe, so `handleSaveSubset`'s `ADD_GLOBAL_STEP`/`derive` block (`DataStudio.jsx:868`, gated on `if (recipe && parentId)`) never runs and the op is logged `reproducible: false`; only Spatial's producers pass a recipe. Fixing the recipe comes before exporting it. **Second finding, folded IN (§7):** checking whether Spatial work reaches the Clean export answered *yes — already unified, nothing to merge* (the `sp_*` column-adders call Clean's own `addStep` via `addStepTo`, `DataStudio.jsx:1055`, and `patch` from the Data Viewer is a pipeline step) — but exposed a live hole in the round-trip that already ships. `pipeline.json` carries **session-local dataset ids** (`rightId` on all 6 merge steps; `gridDatasetId`/`polyDatasetId`/`refDatasetId`/`bufferDatasetId`/`srcDatasetId`/`tgtDatasetId` across 8 `sp_*` steps) and `ImportPipelineButton` validates only `type` + `exprGuard` — so importing into another project **reports success** while those steps fail or no-op at replay. And `patch` matches on `__ri`, which is per-dataset, so replaying foreign patches edits whichever row happens to carry that id: arbitrary cells, silently. Fix keeps single ownership — the dataset-ref fields get declared `type:"dataset"` in `STEP_REGISTRY` and the check is *derived* from it (a hand-maintained list in the importer would be the condition-language dialect drift all over again), with a `pipelineReliabilityValidation.mjs` guard asserting no `*DatasetId`/`rightId` field is left as `"text"`. The `"self"`/`"active"` sentinels mean "current frame" and must NOT be flagged. `patch`/`inject_column` are kept when the payload's `datasetId` matches the target and dropped-with-a-count when it doesn't — the re-import-your-own-pipeline case is real work, and neither silently applying nor silently dropping is acceptable. Also out: importing results, column remapping, `spatial_maps` (goes with derived datasets). **What the subagent-driven build turned up beyond the spec, across two review rounds on `modelSpec.js` and one adversarial pass on `artifactIO.js`:** `applySpec` was originally a MERGE onto the current sidebar rather than a SET — a field absent from the incoming spec kept the previous model's value, so restoring an OLS spec after an HAC one silently kept the HAC `seType`; every field now carries a `def` (dataset-dependent ones, like `factorVars`, as a function of `ctx` — a static default there would have reset every string column to "not a factor" on any spec missing that key). Legacy pins (saved before this table existed) have no `spec.model` key at all — `onRestore` now seeds it from `r.type` and flags legacy pins explicitly rather than silently defaulting to OLS. `estimationDispatch.js` sets `spec.xVars` to the EXPANDED design matrix (factor dummies, interaction products); the naive wiring clobbered it with the raw sidebar list, breaking Sun-Abraham's control-filtering export — fixed by having `collectSpec` emit `xVarsRaw`/`wVarsRaw` via the table's `stateKey` mechanism instead. Spatial weights' `i`/`j`/`w` column fields name columns in the *referenced* weights dataset, not the modelled one, so they're `scalar`, not `column` — declaring them `column` would falsely flag every valid spatial spec as broken. `sp_buffer_exposure` turned out to carry TWO dataset-reference fields (`bufferDatasetId` and `gridDatasetId`); the registry-derived guard caught both without anyone hand-listing either. `artifactIO.js`'s first pass didn't validate that a parsed model's `spec` was an object at all, so a 60-byte hand-edited file crashed the import picker; hardened to 97 checks including deny-by-default vocabulary rejection (pinned by a negative-control test, since a `condToSQL`-shaped `default: return "TRUE"` regression there would otherwise go unnoticed), recursive stripping of session-local identity from nested plot layers, and capped/sanitised error messages. No field was found with a missing setter — all 61 setter names were verified against real `useState` declarations before Task 3 wired them in. |
| 2026-08-17 | `specs/2026-08-17-theme-render-fidelity-design.md` | **DONE 2026-08-17** — build + lint:undef green, browser pass pending Franco | Theme/render upgrade, scope "núcleo: render + tokens + bugs". **The brief it started from was ~70% obsolete**: it asked for a theme system to be built, but `theme.js`/`ThemeContext.jsx` already had tokens, 104 consumers, sans switching, density and a split elevation ladder. The unpolished look was specific defects, not architecture. Biggest single fix: `index.css` was **2 lines** with no font smoothing, so light-on-dark text got subpixel AA (thick, colour-fringed). Fonts were also injected after mount with no `preconnect`, so every cold load painted in `system-ui` then reflowed. **The change re-proved one lesson three times over: a hex literal duplicating a palette value is a bug with a delay fuse.** Refining the palette instantly broke (a) `JOURNAL_MAP`/`PRESENTATION_MAP` in `plotExporter.js`, so a "Journal" export would have silently emitted a dark-background figure; (b) three private copies of two regexes in `ModelPlots`/`ResidualPlots`/`resultDisplay` stripping the ground rect by `fill="#080808"` — all three stopped matching, so every exported SVG would have kept its dark ground (now one palette-derived `stripGroundRect()`); (c) the elevation ladder itself, which hardcoded border hexes and referenced `#2e2e2e`, a colour with no token. Fix is single ownership + derivation, not a careful re-sync — same shape as the condition-language dialect problem. Genuine contrast bugs fixed: `EstimatorSidebar` had 19 DARK hexes baked in as estimator colours (`#7ab896` on white ≈ 2.4:1 in light mode), `#e05c5c` was an orphan red in 11 sites across `ModelingTab`/`didPlots`, `patchDarkTheme()` painted `#9a9a9a` legend text onto white, `textMuted` was ~2.1:1 dark / ~2.8:1 light, and **light mode had inverted depth** (`bg #f4f3f0` > `surface2 #f0eeeb`, so a nested panel sank below the app background). Two bugs found in passing: exported SVG/PNG could never load the page's webfont (a serialized SVG in a blob URL is an isolated document — the family name survives, the file does not), so every "publication-ready" figure used a fallback typeface, fixed by inlining `@font-face` as data URIs (needs `fonts.gstatic.com` in CSP **`connect-src`**, not `font-src` — the recurring trap); and `ThemeProvider` rebuilt `C`/`T` on every render, so no effect could depend on them, which is why PlotBuilder never redrew on a theme switch (now memoised, and `C`/`T` are in its dep array). Warm axis kept at Franco's direction, but the palette is now a `PALETTES` registry with `buildElevation(C, theme)` deriving from it, so a future Slate/Zinc axis is one entry. Strictly additive to the token API; `src/math/` and `src/core/` untouched, and PlotBuilder changes were presentational only (no thresholds, domains or scale logic). **Deferred:** 621 `borderRadius` literals bypassing the `radius` tokens; `GeoPlotCanvas` rendering in `fontFamily: "serif"`. |
| 2026-08-16 | `specs/2026-08-16-factor-reference-category-design.md` | IMPLEMENTATION COMPLETE 2026-08-16 — browser validation pending Franco | Lets the user pick the reference category for a factor variable (X regressors and LSDV's FE dummies only — true FE/TWFE/EventStudy are demeaned, no per-level dummy to choose a reference for). UI: clicking the existing "f" badge on an already-factored `Chip` opens a popover of that column's levels, instead of adding a new right-click pattern (keeps IDEAS.md's Idea 1 parked, per Franco's call). Threaded through **four independently-implemented** dummy-building sites (`helpers.js` `applyFactors`/`expandInteractions`, `duckdbFactors.js`'s SQL path, `PanelEngine.js`'s `runLSDVMulti`) plus three exporters. **Found mid-implementation: each exporter actually had a SECOND independent copy of the same "wrap factor in syntax" logic** (`fmtR`/`fmtPy`/`fmtS` inside `transpileModel` for 2SLS/GMM/LIML vs. `buildRFormulaStr`/`buildPyFormulaStr`/`buildStataVarlist` for the common OLS/WLS/Logit case) — fixing only the first, as planned, would have shipped a feature invisible on the most common estimator; consolidated into one real function per language. Stata's `ib(#).` only works for a numeric reference (Stata's operator takes a literal value, not a level name) — surfaces the pre-existing `i.col`-on-a-string-column gap, documented not fixed. LSDV's FE reference turned out to be export-scope-OUT entirely (not just deferred): all three exporters represent LSDV as absorbed FE with no reference concept; the Litux engine still honors the choice correctly, just nothing to export it into. R-validated against `relevel(factor(x), ref=)` (6 pairs, 1e-6/1e-4) plus two model-invariance identity checks. |
| 2026-08-04 | `plans/2026-08-04-condition-language-surfaces.md` | **DONE 2026-08-06** — 8/8 tasks, browser pass pending Franco | Slices 3-8 of the condition-language spec: the UI surfaces. 8 tasks. Two things writing it turned up. (1) **A seventh dialect**, not the six the spec counted: `matchOne` inside `grouped_mutate` (`runner.js:1224`) has two aliases nothing else uses (`=`, `<>`) and a *lenient* equality — `String(rv) === String(val) || rv === nv`, so `== 10.0` matches `10`, where canonical `eq` is text-only. Migrating it is a documented behaviour delta. (2) **All three replication exporters emit the subset operator verbatim** (`rScript.js:1636`/`1785`, `pythonScript.js:1292`/`1459`, `stataScript.js:1192`/`1340`, plus the bundle README at `replicationBundle.js:197`). That works today only because SubsetManager's six operators are simultaneously valid in R, Python and Stata; switching to canonical ids without touching them would emit `region eq "north"` into an R script. New `opInfix(op, lang)` translates and **throws** for operators with no infix form (`in`, `contains`, the null ops), so widening SubsetManager's vocabulary later fails at test time instead of shipping a script that does not run. SubsetManager deliberately keeps exactly its current six operators. Task 8 adds a repo-wide guard against an eighth dialect appearing (negative-controlled). **Executing it turned up a fifth permissive default, in the highest-stakes place:** all three `filter` step translators in `stepTranslators.js` read `step.op` only — undefined for the compound trees `FilterBuilder` actually emits — and defaulted to `TRUE`/`True`/`1`, so **every filter built in Clean was silently dropped from the exported R/Python/Stata script**. Even flat filters hit it for 8 of the 15 operators. Verified by running the translators, not by reading them. Fixed with a new `predicateExport.js` (tree-aware, all operators, throws instead of guessing), which also had to handle three NA traps a naive translation would miss: `NA %in% c(...)` is FALSE in R so a bare negation KEEPS null rows, `~isin()` is True for NA in pandas with the same effect, and a pandas mask carrying `pd.NA` cannot be used for boolean indexing at all. |
| 2026-08-04 | `plans/2026-08-04-floating-panels.md` | **DONE 2026-08-06** — 9/9 tasks, browser pass pending Franco | Implementation plan for the floating-panels spec. 9 tasks. Notable finding while writing it: **`PlotCanvas` (`PlotBuilder.jsx:593`) already accepts exactly the 16 fields a saved plot entry carries** (compare `currentPlotEntry()` at `:1643`), so the viewer needs no new rendering path and no `chromeless` prop — just `export` on one line. Maps/models are a documented v1 boundary (summary card + open action) because `MapCanvas` drives a Leaflet lifecycle inside a resizing panel. |
| 2026-08-04 | `plans/2026-08-04-condition-language-engine.md` | **DONE 2026-08-04** — 5/5 tasks, browser pass pending Franco | Slices 1-2 of the condition-language spec (engine only, no UI file touched). 5 tasks. Two things surfaced during execution that the plan had not predicted. (1) **`set_where`'s `empty`/`notempty` meant "null OR empty string", but `isna`/`notna` mean strictly null** — mapping the legacy aliases onto isna would have silently stopped a saved fill-the-blanks step from touching `""` cells. Added `isblank`/`notblank` as distinct canonical ops and pointed the legacy aliases there, so both meanings survive; pinned by tests. Four smaller deltas accepted as corrections and also pinned: string ops become case-insensitive for set_where (filter already was), and null stops matching comparisons (legacy coerced it via `Number(null)`, so `null > -1` was true). (2) **`condToSQL` was not a latent hazard, it was a live bug.** `filter` is in `SQL_STEPS` and `applyStepSQL` got the raw step with no operator guard, but condToSQL read `cond.val`/`cond.conditions` while real steps carry `value`/`predicate`, and its operator vocabulary intersected the runner's on exactly one entry (`contains`). Everything else hit `default: return "TRUE"` — so on any DuckDB-backed dataset a structured filter step returned **every row**, with the step reporting success so no JS fallback fired and nothing was logged.  Two traps documented that any SQL compiler must respect or the paths silently diverge: `evalPredicate` compares `eq` as **text** (`runner.js:246`), so SQL needs `CAST(col AS VARCHAR)` or "10.0" matches 10 in SQL but not JS; and the string ops are **case-insensitive** in JS (`runner.js:271-278`), so SQL needs `ILIKE` and `regexp_matches(..., 'i')`. Also found: the JS evaluator has the SAME silent-default bug as `condToSQL` — `runner.js:280` ends in `return true`. Both now throw. UI slices 3-8 are a separate plan, written after this lands, because they consume the `OPERATORS` table this one defines. |
| 2026-08-03 | `specs/2026-08-03-floating-panels-design.md` | **DONE 2026-08-06** (plan), **AMENDED 2026-08-22** — the spec row itself had gone stale at OPEN long after `plans/2026-08-04-floating-panels.md` shipped; corrected here | From IDEAS.md Idea 5. Generalises `DistinctValuesPanel`'s floating/non-modal/minimizable pattern into a reusable `<FloatingPanel>` + a `PanelStackContext`, and ships two consumers: a new app-scoped **Artifact Viewer** (saved plots/maps, ◀ ▶ through the order `artifactOrder.js` already persists — so the panel shows the same sequence the replication bundle emits) and the migrated module-scoped Distinct Values panel. **Key constraint, already documented at `WranglingModule.jsx:853-863`: `display:none` on an ancestor hides `position:fixed` descendants, and App keeps tab panels mounted that way (`App.jsx:2785`/`3215`) — so a panel mounted inside a module is invisible whenever that module isn't the active tab.** Correct for distinct-values, fatal for a plot viewer. Resolution: **scope is not a prop, it's the mount point** — module-scoped panels mount inside the module, app-scoped mount in a single App-level `PanelHost`. Two consumers with different scopes chosen deliberately so the abstraction is exercised at both extremes. OUT of v1: drag/resize (stacking exists to avoid window management), `AuditTrail`'s blocking modal (a different pattern on purpose), real OS windows via `window.open` (severs the React tree, inline theming and the DuckDB singleton). Only genuinely new logic is the stack offset math over *visible* panels — a hidden-but-registered panel must leave no gap — and that gets the one new harness. **2026-08-22 amendment (Franco's report):** the panel said "No saved plots or maps" while a pinned Explore Time Series analysis sat right below it in `ExplorePinBar`. Root cause: `getExplorePins`/`saveExplorePins` — a third sibling of `getPlotHistory`/`getMapHistory`, same file, same store, same project-scoped `pid` — already existed and was never wired into this panel's fetch. Not a regression; scoped out at v1 because Explore pins (`{kind, label, params}`) have no shared renderer the way PlotBuilder's uniform `layers[]` does, and carry no `datasetId` to resolve rows against. Fixed by giving them the same "summary card + open" treatment Maps already use for the identical reason (§5 of the spec), reusing `ExplorePinBar`'s own `KIND_ICON` (now exported) and each pin's own already-human-readable `label` — no new per-kind copy. **Found in passing, deliberately not fixed:** the spec's own artifact-content table and the plan's task notes both describe "maps/models" sharing the v1 summary-card boundary, but no `kind:"model"` branch was ever built — pinned models are used only for replication-bundle *ordering* (`ReportingModule.jsx`), never for browsing in this panel. Left alone because pinned models already have a dedicated, richer browsing surface (`ModelBufferBar`'s own ◀ ▶, labels, restore-into-sidebar) that a bare summary card would duplicate rather than improve — a scope call for Franco, not a bug. |
| 2026-08-03 | `specs/2026-08-03-unified-condition-language-design.md` | ENGINE DONE 2026-08-04 (slices 1-2) + the §8 Data Viewer bug fixed; surfaces plan OPEN — spec approved by Franco, implementation plan not yet written | From IDEAS.md Idea 6. Grading that idea surfaced a bigger problem Franco named directly: the same "compare a column against something" operation is spelled **five** ways in the UI (Explore renders `=`, SubsetManager renders `==`, Clean says `is null`, the Data Viewer says `empty`) and implemented **six** times in code (`evalPredicate` in runner.js, a duplicate of it in `CleanTab.jsx:772`, `buildPredicate` for `set_where`, `condToSQL` in duckdbRunner, the Data Viewer's inline closure). Fix: one canonical `src/pipeline/predicate.js` — `OPERATORS` table, `evalPredicate`, `predicateToSQL`, `normalizeOp` — adopting the predicate tree the `filter` step already persists, so the largest class of stored pipelines needs no migration. **The load-bearing change is that `predicateToSQL` throws on an unknown operator instead of `condToSQL`'s `default: return "TRUE"`, which today silently matches every row and returns a plausible wrong table.** Read-time `normalizeOp()` is mandatory (not optional) because pipelines also live in user-exported `.json`, E2EE sync blobs and other people's shared projects, none of which a migration script can reach; aliases are never deleted. Explicitly OUT: Calculate's equation pad (`=` there means "solve this relation", a different language) and JS calculation expressions in mutate/Simulate (the scrubbed worker is the audited security boundary). Includes the HintBox updates for Data/Clean/Explore/Model per the CLAUDE.md convention, plus a new line in Calculate's HintBox stating that its `=` is an equation — a confusion this change creates. Slices 1-2 are pure refactors with zero user-visible change, on purpose. |
| 2026-07-30 | `specs/2026-07-30-distinct-values-panel-design.md` + `plans/2026-07-30-distinct-values-panel.md` | IMPLEMENTATION COMPLETE 2026-07-30 — browser validation pending Franco (see the checklist in the plan's Task 6) | From today's Supabase feedback batch — a read-only "view distinct values of a column" utility for Clean > Formatting, not a pipeline step. `getDistinctValues(tableName, col)` in `duckdb.js` mirrors the existing `computeColStats` pattern (SQL on the full dataset, JS fallback when no DuckDB table), sorted by frequency descending, capped at 500 distinct values. UI is a new pattern for this app: a floating, non-modal, minimizable panel (not the existing full-screen dimmed-overlay modal `AuditTrail.jsx` uses) — the point is to stay visible as a reference while the user keeps working elsewhere, e.g. while building a `country_code` mapping (overlaps with the country-code-transform spec's noted use case). Single instance, no drag/resize, no copy/export in v1.
| 2026-07-28 | `specs/2026-07-28-replication-script-completeness-design.md` | OPEN — spec approved by Franco, implementation plan not yet written | From today's Supabase feedback batch. Part A: PlotBuilder's standalone "copy script" (Explore tab, free mode) never emitted a dataset load line for any of R/Python/Stata — narrower root cause than the original report suggested, since the Report-tab unified script and model-embedded plots already load correctly through separate paths. Fix threads `filename`+`loadOpts` from `ExplorerModule.jsx` into `PlotBuilder.jsx`, which prepends a load line via the existing `buildRLoadLine`/`buildPyLoadLine`/`buildStataLoadLine` (`loadLine.js`) when there's no `scriptPreamble`. Part B: Simulate's replication script only covered the DGP; unify by walking the existing cross-module `sessionLog` (already populated by StatWorkspace/QTEPanel for exactly this purpose) and reusing each op's already-existing generator (`generateStatInferenceScript`, `generateCalcScript`) — plus closing a found gap where the Distributions section neither logs nor exports a script at all. Session-boundary fix: only log entries after a mount-time marker, since the log is a persisted project-wide timeline, not scoped to one tab visit.
| 2026-07-28 | `specs/2026-07-28-country-code-transform-design.md` | IMPLEMENTATION COMPLETE 2026-08-15 — browser validation pending Franco | New Feature-tab pipeline step `country_code` (ISO2/ISO3/name/**continent** conversion — continent added 2026-08-15 to unblock LMU PS4's `countrycode(..., "continent")`, 5-way taxonomy matching R's default), analogous to R's `countrycode`. Reuses the `recode`/`normalize_cats` pattern: alias matching (199-entry `countryCodes.js` table) happens once in `FeatureTab.jsx` and is frozen into a literal `step.map` — runner.js and R/Python/Stata exports never see the alias table, only an exact dictionary lookup. `internal: true` in the registry (excluded from NL catalogue, same mechanism as `sp_*`/`patch` — the AI can't safely fabricate `map` without the dataset's real distinct values). Unmatched values → `null` + a mandatory full-table preview showing matched/unmatched counts before Apply. Scope v1: ISO2/ISO3/name/continent only, no region/numeric-ISO, no fuzzy/Levenshtein matching beyond the curated alias table. **Found on the way, flagged not fixed:** `rScript.js`/`pythonScript.js`/`stataScript.js` each carry a full second copy of `stepTranslators.js`'s per-step switch (delegating to the shared file only for `sp_*` steps) — every new step type needs its translator case written in both places. Same duplication-risk class CLAUDE.md tracks for the condition language.
| 2026-07-25 | *(no spec yet)* — Fuzzy RDD covariate support | OPEN — deliberate scope cut taken 2026-07-25; needs R fixtures before any implementation | **Fuzzy RDD currently estimates UNADJUSTED everywhere, by design.** Found while fixing the dead RDD X picker: the JS `runFuzzyRDD` (`CausalEngine.js:636`) destructures only `{bandwidth, kernel, seOpts, polyOrder}` and takes no controls, while the SQL `runFuzzyRDDFromSuffStats` (`RDDSuffStatsEngine.js:115`) accepted and used `controls` — so the SAME spec was covariate-adjusted above the ~50k-row SQL threshold and unadjusted below it, silently. Worse, `FuzzyRDD` is in neither `SHOW_W` nor `FuzzyRDDConfig`, so those covariates could never be chosen deliberately — the SQL path was picking up leftover `wVars` from whatever estimator ran before. Both `wrapResult` call sites also recorded `wVars` in the spec despite the JS estimate ignoring them, which propagated a false "adjusted" claim into the replication script. **Fix taken (option B, Franco's call):** covariates are Sharp-RDD only; the Fuzzy SQL branch passes `controls: []` and neither path records `wVars`. `FuzzyRDDResults`' `r.spec?.wVars ?? []` fallbacks now correctly report no covariates. **Deferred (option A):** real covariate support in both paths + a W picker for Fuzzy RDD. Legitimate and standard (`rdrobust`'s `covs`; CCFT 2019), and it would remove the odd asymmetry with Sharp RDD, which already supports controls via `safeControls`. **Blocked on R validation:** `fase7Benchmarks.json` contains ZERO covariate cases (verified 2026-07-25), so the SQL implementation that was just disabled was never R-validated — re-enabling it without `rdrobust(..., covs=)` fixtures would be reintroducing unverified math. Implementation note for whoever picks this up: controls must enter BOTH the first-stage and the reduced-form designs in `runFuzzyRDD`; adding them to only one biases the Wald ratio with no visible error, and the delta-method SE must propagate through both. |
| 2026-07-25 | `specs/2026-07-25-split-screen-panes-design.md` + `plans/2026-07-25-split-screen-panes.md` | IMPLEMENTATION COMPLETE 2026-07-25 — browser validation pending Franco (⊞ split open/swap/drag/dblclick/min-width-close, sidebar auto-fold under 700px in Clean+Model, reload restores layout per project, single-pane mode identical to before) | Side-by-side panes so a user can work in two modules at once (Clean + Model), with a Windows-snap-style adjustable divider. **Key finding that makes it cheap:** `App.jsx:3030` already mounts every tab panel simultaneously (`display:none` toggling, to preserve state) and each panel is already `position:absolute; inset:0` (`tabPanel`, `App.jsx:3220`) — so the split is a pure CSS box change (`left`/`width` per pane), with **no reparenting, hence no remounts and no module-state loss**, and a single pane renders identically to today. Free consequence: a module is one DOM element, so showing the same module twice is structurally impossible — assigning an already-visible module to the other pane swaps them, sidestepping duplicate-`DataStudio` collisions on `studioRef` / `litux:wrangle_tab:${pid}` / debounced IDB writes. Per-tab dataset selection already exists (`activeDatasetIds[tab]`, `App.jsx:2847`), so Clean-on-A + Model-on-B works. Interaction: ⊞ toggle in `WorkspaceBar`, focused pane takes the tab click, draggable divider (50/50 default, magnetic snap at centre, double-click reset, ~360px min width → past it the pane closes). **The real work is width, not the split**: `History.jsx:84` (230px) and `ModelingTab.jsx:2019` (300px) are `flexShrink:0`, so each self-observes via `ResizeObserver` and folds below ~700px — Phase 1, which ships value with no split at all. Phases: (1) collapsible sidebars, (2) two-pane split, (3) N panes (optional, uncommitted). Sequenced **before** Tauri packaging, which is a wrapper that would otherwise ship a UX we'd immediately rework. Known MVP limitation: modals (Standardize, AuditTrail) stay viewport-centred, not pane-confined. |
| 2026-07-22 | `specs/2026-07-22-explore-sql-fastpath-design.md` | IN PROGRESS — Phases 1/1b/3/4/5/6 DONE (1b 2026-09-17: Summary stats/head/tail + column info in SQL, lazy row materialisation), Phase 2 (filter → SQL WHERE) PENDING | Evidence Explorer's stats (Group & Summarize, Summary Table, Correlation, Histograms, Time Series, PlotBuilder scatter) computed off a JS row array that was either a 500-row DuckDB preview (wrong numbers, silently) or a full-table JS materialization (correct but doesn't scale to 900k+ rows, against the DuckDB performance strategy). Fix is pushing each tab's computation into SQL so the JS↔DuckDB boundary only ever crosses aggregated results. **Done 2026-07-22:** Group & Summarize (`GROUP BY` query), Correlation (`corr()` matrix), Distributions histogram+categorical (stats/bins/counts all via SQL, `SvgHistogram` accepts precomputed bins), Time Series (`GROUP BY` per period/group, reused for ACF/PACF/ADF's flatY), and Plot Builder (SQL `USING SAMPLE` past 50k rows, disclosed via banner, scoped at the `ExplorerModule` call site rather than restructuring `PlotBuilder.jsx`'s shared-dataset marks). **Still open:** Summary Table (Stats/head/tail view — `statsFor()` called from many render/export sites, needs a bigger restructure) and translating active QuickFilter conditions into a SQL `WHERE` (every phase above falls back to JS when a filter is active). |
| 2026-07-16 | `specs/2026-07-16-spatial-pipeline-integration-design.md` + `plans/2026-07-16-spatial-pipeline-integration.md` | CODE-COMPLETE 2026-07-16; **`sp_*` R/Py/Stata translators DONE 2026-07-17** — browser-validation pending Franco | Spatial Analyze ops join the non-destructive per-dataset pipeline (shared with Clean, single execution timeline). Hybrid model: column-adders (Distance, Buffer, Grid Assign, Spatial Join, Nearest, Boundary Dist, CRS; Geocode reuses existing step) become `sp_*` pipeline steps committed via preview → "Add to pipeline" (`onAddStep` mirroring `addInjectColumnStep`); dataset-producers (Aggregate to Grid, Areal Interp, Metric Buffers) keep save-as-dataset but route through `handleSaveSubset(..., recipe)` so the existing derive-edge (`ADD_GLOBAL_STEP opType:"derive"`) records the derivation for topo-ordered workspace scripts. Runner cases call existing pure `SpatialEngine.js` fns; secondary datasets resolve via `context.datasets[id]` like `join`; registry category `"spatial"`; `sp_*` excluded from NL catalogue for now. **R/Py/Stata translators DONE 2026-07-17** (`spatialR`/`spatialPy`/`spatialStata` in `pipeline/stepTranslators.js`, dispatched by a `sp_*` prefix check from both `toR/toPython/toStata` and each export file's local `transpileStep`; Stata = documented comment per the never-silently-drop rule; export-integrity harness 329/0; build + lint:undef green). |
| 2026-07-14 | `specs/2026-07-14-share-notification-toast-design.md` | OPEN — spec approved by Franco, implementation plan not yet written | Follow-on UX for the 2026-07-14 share RLS fix (see BugTriage 2026-07-06 row): (1) remove the redundant share-link/Copy/email-client UI from `DatasetManager.jsx`'s "Share created" confirmation — the recipient already has a working one-click "Shared with me" import list in `App.jsx`; (2) new auto-dismissing 5s toast "{owner email} shared a project with you" on the recipient's next app load/sign-in only (no polling, no Realtime — Franco's choice). Requires: new `owner_email` column on `project_shares` (denormalized, avoids a new cross-table `profiles` RLS grant), a new small IndexedDB store `notified_shares` for one-time-per-share toast tracking, and a new `ShareNotificationToast.jsx` component. Plain toast, no click-to-navigate (Franco confirmed keep it dumb). |
| 2026-07-10 | `plans/2026-07-10-panel-nway-fixed-effects.md` | DONE 2026-07-11 (R-validated, 18/18) — post-hoc UX fixes also landed | From Supabase feedback: users can't choose FE columns for TWFE/FE/LSDV/EventStudy beyond the single entity/time pair declared in PanelTab, and can't cross two columns into one FE interaction group. 8-task plan: (1) generalized N-way alternating-projection demeaning engine `PanelWithinEngine.js` (same Gaure/Correia method already proven in `NonLinearEngine.js`'s `runPoissonFEMulti`), (2) FE interaction column materialization, (3) `PanelTab.jsx`/`validatePanel` gain `feCols[]` (entity/time kept as `feCols[0/1]` for backward compat), (4) `PanelEngine.js` FE/FD/TWFE/EventStudy/LSDV rewritten with `*Multi` variants + legacy wrappers, (5) UI wiring (`ModelConfiguration.jsx`/`ModelingTab.jsx`/`estimationDispatch.js`), (6) DuckDB SQL fast-path explicitly capped at 2 FE dims with a fallback guard (documented scope cut, not silent), (7) R/Python/Stata replication export for N-way absorption, (8) R validation. Brainstormed + scoped with Franco 2026-07-10 (chose "Full N-way FE" + "include FE interactions now"). **R validation (2026-07-11):** Franco ran `panelNwayFeRValidation.R` — 18/18 checks pass (2-way independent, 3-way independent, nested state+state×year, coef/SE/df/n each) at standard tolerance; the nested-case df-formula risk flagged in the plan did not materialize (`feDegreesOfFreedom()`'s additive formula matches fixest's own reported df=183 exactly for this fixture). **Post-hoc fixes found during Franco's browser testing (2026-07-11/12), all landed on the same branch:** (a) whole-feature review before merge caught two critical gaps the per-task reviews missed — FE interaction picker was fully inert end-to-end (never wired to `materializeFEInteraction`), and LSDV/EventStudy replication export silently truncated to entity+time despite the in-app estimate using the full N-way set; (b) `FEColumnPicker` silently hid whenever `panel.feCols` wasn't yet committed (needed a fallback to entity+time); (c) `WranglingModule.jsx`'s `onReady`/`proceed` handlers manually whitelisted the `panelIndex` payload fields and silently dropped `feCols`/`interactionCols` before they ever reached the Model tab (file untouched by any of the 8 tasks — a classic "gap between tasks" bug); (d) FE interaction needed to become a genuine per-estimation toggle mutually exclusive with its two raw source columns, not a locked always-on chip; (e) LSDV's legacy separate "Time Fixed Effects" toggle was removed entirely — it was silently overriding the new Fixed Effects picker's "time" selection and could combine with the picker in ways that produced a singular-matrix error; the picker is now LSDV's sole FE source, same as every other estimator. Still open (deferred, not blocking): replication-script export doesn't yet reflect the FE-interaction picker (tracked separately, `task_4261a416`-class follow-up); per-dataset pipeline persistence and PlotBuilder dataset-binding issues flagged earlier this session remain separate, unrelated follow-ups.|
| 2026-07-08 | `specs/2026-07-08-report-script-persistence-design.md` | OPEN — spec approved by Franco, implementation plan not yet written | AI Unified Script (Report tab) persistence + manual editing + incremental AI update. Scoped down from Franco's original ask ("save scripts to Google Drive / local directory") to a privacy-first, Litux-local design during brainstorming: new IDB store `report_scripts` (v10) keyed by pid, one entry per language; artifact-ID comment marking on model/plot/map sections so an "Update" action can diff new-vs-tracked artifacts and only append what's missing, never touching hand-edited sections; orphaned (unpinned/deleted) artifacts get a one-time comment, never silently removed; manual edits persist via an explicit solid-styled "Save" button (no autosave); "Reset" is `ConfirmPopover`-gated. `CodeEditor.jsx` (ModelingTab's separate per-model script viewer) is out of scope. |
| 2026-07-02 | `specs/2026-07-02-calculate-limit-tool-design.md` | CODE-COMPLETE 2026-07-08 — browser-validation pending Franco | Small additive feature from ClaudeFB.md 2026-06-19 "Add limits" — a 6th Calculate-tab tool (`lim`) computing numeric `lim(x→a) f(x)`, finite point or ±infinity, mirroring the existing Derivative tool's UI/engine pattern exactly. Executor: Claude Fable 5. |
| 2026-06-16 | `plans/2026-06-16-unified-artifact-registry.md` | IMPLEMENTATION COMPLETE — browser validation pending Franco | All 10 tasks landed (commits fd32b9b6→c03d0410) with per-task spec + code-quality review. 10-task implementation plan for the unified-artifact-registry spec (lint:undef + build gates per task; Franco browser-validates). Tasks: (1) artifactOrder store+helpers, (2) trimResult datasetId + buffer setLabel/reorder, (3) Report-after-refresh App-level buffer load + unlock, (4) model datasetId tag + ModelBufferBar rename/reorder/switch, (5) PlotBuilder project-scope+rename/reorder/request-dataset, (6) ExplorerModule projectPid+handoff, (7) App pending-plot handoff, (8) Spatial map reorder, (9) Report global-order panel drives unified script, (10) docs. |
| 2026-06-16 | `specs/2026-06-16-unified-artifact-registry-design.md` | IMPLEMENTATION COMPLETE — browser validation pending Franco | Unified project-artifact registry over the existing 3 stores (plotHistory/mapHistory/model_buffer). Fixes: (1) Report locked after refresh — App-level `modelBuffer.setProject(pid)` seeds `modelingSession.pinnedModels`, unlock Report when output OR pinned models exist, render Report from models when no live dataset; (2) Explore plots vanish on dataset switch — re-key plotHistory + explore pins by **project pid** (was dataset id), tag each plot with `datasetId`, best-effort merge legacy per-dataset entries; (3) rename for plots+models (maps already), per-module reorder + a `artifactOrder_<pid>` global-order array Report drag-sets to drive the single R/Py/Stata script; (4) click plot/model → auto-switch active dataset (models in-place via `onSwitchDataset`, plots via App pending-plot handoff). Translation (plot→ggplot/matplotlib/Stata, map→leaflet R/folium Py, model→R/Py/Stata) + Report cross-dataset aggregation already exist — this is UI scoping + ordering, not a translation/storage rewrite. Brainstormed + design-approved 2026-06-16. |
| 2026-06-14 | `plans/2026-05-22-fase-x5-bug-bash.md` | IMPLEMENTATION COMPLETE — browser validation pending Franco | Fase X5 feedback/UX sweep. ClaudeFB fully reconciled against current code; BugTriage has no unverified `open` rows. Fixes: resizable PlotBuilder + frozen OLS/WLS diagnostics (`35f4b5a6`), Leaflet heatmap layer (`5e105529`), IV-Poisson IRR interpretation (`bc2d7b9d`), theme-token audit (`a433e0c5`). Deferred enhancements have explicit `fix-later` rows. `lint:undef` + build green; no browser-dependent item marked validated. |
| 2026-06-14 | `plans/2026-05-22-fase-x4-performance.md` | IN PROGRESS — implementation complete; browser validation pending Franco | Fase X4 of the pre-launch roadmap (Track C). COEP corrected `credentialless`→`require-corp` for SharedArrayBuffer portability (credentialless unsupported in Safari/Firefox Android); OPFS registry/hydration now restores a fresh DuckDB table on project reopen without re-import and awaits durable Parquet writes; temporary DuckDB handles/buffers cleaned up. Node harness `src/services/data/__validation__/faseX4PerformanceValidation.mjs`: **10/10 green**, LRU stays at 50 and realistic k=20 entries serialize to **431.8 KiB** (<10 MiB). Deployed-browser harness/checklist + target-only benchmark JSON added for OPFS ≥10× warm start, 8 scenarios ×20 P50/P95, and 100-estimation heap growth <100 MiB. No browser numbers claimed; Franco must run `?validation=faseX4`. **2026-07-02: found + fixed a likely root cause of prior "large file never finishes loading" reports on the deployed site** — `vercel.json` CSP `connect-src` was missing `cdn.jsdelivr.net`, which silently blocked DuckDB-Wasm's `.wasm` fetch for any file >10MB (local dev doesn't apply `vercel.json` headers, so this was invisible outside a real deploy). Fixed in commit `4637b27f`; re-run `?validation=faseX4` + a real large-file load after redeploy to confirm. |
| 2026-06-27 | `plans/2026-06-27-callaway-santanna-rewrite.md` | IMPLEMENTATION COMPLETE — R validation pending Franco | Claude implementation plan (10 tasks, review gate per task). Supersedes the Codex handoff prompt from 2026-06-14. All 10 tasks landed; 2026-07-02 follow-on polish (commits `c1e8e213` plot sizing/axes/nice-tick-domains/table readability, `58ff5490` PNG export per plot + LaTeX export for the group-time ATT table) closes out the UI/UX layer. Only remaining gap: Franco runs `callawayRValidation.R` → `callawayBenchmarks.json`. |
| 2026-06-14 | `specs/2026-06-14-staggered-did-callaway-santanna-design.md` | IMPLEMENTATION COMPLETE — R validation pending Franco | **Spec 1** of decomposed staggered-DiD effort. Rewrite Callaway-Sant'Anna to match R `did` (root causes: covariates ignored, universal vs varying base period, hand-rolled SE, only dynamic aggregation). Builds a **shared estimator-agnostic staggered-DiD framework** (`src/math/did/`: `drdid.js` 2×2 DR/reg/ipw building block + `staggeredDiD.js` base-period/control/4-aggregations/IF-bootstrap-uniform-bands + rewritten `CallawayEngine.js`) reused by later estimators. Implements `did` args: est_method {dr,reg,ipw}, control {never,notyet}, base {varying,universal}, anticipation, xformla. Adds 4 ggdid plots incl. faceted group-time. Reorgs estimator menu into **Panel / DiD / Event Study** (SunAb & CS in both DiD+EventStudy; surfaces SunAb which was engine-only; CH added as `planned`). JS-only, ~200k-row guard. Franco runs R for validation (6dp coef/4dp analytic SE; bootstrap bands structural). Brainstormed + design-approved 2026-06-14. Implementation plan written as a Codex handoff prompt: `plans/2026-06-14-staggered-did-callaway-santanna.md` (10 TDD tasks, review gate per task; Franco runs R fixtures). |
| 2026-06-14 | `plans/2026-06-14-spatial-replication.md` | DONE 2026-06-14 (structural; sf/geopandas runtime pending Franco) | Closes the last replication-fidelity gap: spatial **analyze** ops (join/buffer/grid/distance — 11 logged opTypes) were non-reproducible (unified script emitted only a comment; R validation 2026-06-14 had to "export schools_barrios.csv from Litux"). New `src/services/export/spatialScript.js` `transpileSpatialOp(opType, params, lang, datasets)` → R sf / Python geopandas; wired into ReportingModule per-execution assembler (was the line-996 comment). The recipe was already captured (each section logs `appendLog({module:"spatial",opType,params})`) — this just translates it. Stata = documented comment (no native geometry). Claude's lane; collision-free with Codex's X5 (map/ heatmap + visual polish). |
| 2026-06-14 | Staggered-DiD roadmap (no spec yet) | OPEN | Spec 2 = de Chaisemartin & D'Haultfœuille (CH, "Staggered DiD" menu entry). Later: BJS (2021-24), Gardner 2SDID (did2), Liu-Wang-Xu (2022), Wooldridge flexible TWFE / Poisson DiD (Poisson engine exists). All plug into the framework built in Spec 1 (engine = only new code). Also deferred: repeated cross-sections, DuckDB large-n CS path. |
| 2026-06-12 | `specs/2026-06-12-replication-fidelity-design.md` | IN PROGRESS — Fase 0 DONE (browser-validated; loadOpts-hydration fix 4afcb61); Fase 1 code-complete (timeline IDB 289baa37 + Explore pins f9e8872, browser-validation pending); Fase 2 code-complete (derivation f057078f + spatial ordering d7a96868 + manual edits 51819010 + multi-model d4bc3d4c, browser-validation pending Franco); stepTranslators.js code-complete 2026-06-30 (all 21 previously-missing step types now emit real R/Stata/Python in all 3 translators, 1953 lines, 0 unknown-step fallbacks for real types — node smoke-test green; browser-validation pending Franco); next Fase 3 / Track P | Single-source spec + execution plan for full R/Py/Stata replication fidelity. Executors: Fable 5 → Codex (handoff protocol in §7). Scope: unified script = Data/Clean/Model/Spatial(+plots); Calculate/Stat/Simulate excluded (mini-scripts only). 8 locked decisions (§2): D1 AI never reorders (current prompt rule 2 = the bug), D2 manual-edit warning per-language (R/Stata download cleaned CSV, Py emits cell edits), D3 structuring question (Per-module/Per-execution[Fase3]/Custom), D4 derived subsets = derivation edge on globalPipeline (hybrid, recipe-first), D5 instrument Explore fully (save/pin→artifact, exact args), D6 plotConfig→ggplot translator first, D7 Spatial Map via R `leaflet`+Py `folium` (needs named saved-maps history), D8 argument fidelity bar. Phases: Fase 0 (prompt no-reorder + structuring Q + manual-edit warning) → Fase 1 (persisted timeline + Explore instrumentation) → Fase 2 (derivation edges + spatial ordering + honest patch handling + multi-model export) → Fase 3 (per-execution-order + interleaving detection). Track P (plots, parallel): P1 ggplot translator → P2 spatial geo → P3 map leaflet/folium → P4 model plots (broom::augment/tidy) → P5 matplotlib+Stata. Prereq context already landed 2026-06-12: multi-dataset unified-script fix (row 5b below) + dataset rename. |
| 2026-06-11 | `plans/2026-06-11-litux-website.md` | DONE — LIVE at litux-site.vercel.app (2026-06-11) | All 14 tasks executed 2026-06-11 via subagent-driven development (18+ commits). Site lives in SEPARATE repo `github.com/1francom/litux-site` (private; local `C:\Franco\litux-site`). Astro 5 + React islands; 9 Vitest tests green; 12 static pages + `/api/notify` server route; client bundle verified free of Supabase key leakage. Review fixes applied: one-shot tour reveal, aria-pressed/aria-current/aria-label/role=status a11y set, h2 step titles, simulate video entry, .env.example. **Deployed + validated 2026-06-11:** Vercel project live at litux-site.vercel.app with env vars set (gotcha hit and fixed: first attempt used the publishable key → 502; needs the SECRET/service_role key, and a REDEPLOY after env changes since Astro inlines env at build time). `desktop_waitlist` table created via Supabase MCP migration (RLS on, zero policies = service-role-only by design; advisor INFO is expected). Waitlist E2E-verified: POST /api/notify → 200 → row in table; test row cleaned. `APP_URL` confirmed 2026-06-12 → `litux-c51o-iota.vercel.app` (the app's public Vercel alias; the three "try without signing up" CTAs append `?guest=1` → guest mode, see guest-mode row below). Known follow-ups: ModuleTour blurbs + /start steps copy still in components (move to content files with i18n work); transparency section is cards not a drawn diagram. |
| 2026-06-14 | IV-Poisson fix + Explore pin bar (no spec — bugs/feature, inline 2026-06-14) | DONE — browser-validation pending | **IV-Poisson fix (commit 4ff21467):** `wrapResult("IVPoisson")` fell through to default branch → empty result shell. Added `wrapIVPoisson()`, `ESTIMATOR_META.IVPoisson`, `FAMILY_MAP.IVPoisson`, `buildFormula` case. **Explore pin bar (commit 4ff21467):** new `src/components/explore/ExplorePinBar.jsx` — two-row sticky bar (Plots / Tables); each `PinBtn` click now stores to both the replication timeline (`appendLog`) and a local `pinnedItems` state; cards show kind icon + label + ×; ⊞ Compare (2+ same-kind selected) shows side-by-side params for plots or merged mean/sd table for summary stats. AI coach dispatch re-review pending Franco. Callaway-Sant'Anna browser-validation pending. |
| 2026-06-13 | Guest mode + dashboard logout (no spec — small feature, brainstormed inline 2026-06-12/13) | DONE — browser-validated 2026-06-14 | "Try without signing up": app renders with `user=null` and the login gate bypassed when a guest flag is set; full local core works (IndexedDB), only cloud sync needs an account. 5 app files (commit 9b1b6221): NEW `services/auth/guestMode.js` (localStorage `litux_guest` flag + `consumeGuestParam()` reads/strips `?guest=1`); `AuthContext.jsx` reactive `guest` state (lazy-init = no login flash) + `enterGuest`/`exitGuest`, real login supersedes guest; `AuthGate.jsx` `!user && !guest` → LoginForm; `LoginForm.jsx` "Continue without an account" button; `WorkspaceBar.jsx` sign-out exits guest. Website (litux-site commit 6101553): `APP_URL_GUEST` on the three try-CTAs. Dashboard logout (commit bce535f4): Cloud-projects card header shows Sign out next to email, or "Guest session · Exit" in guest mode — so logout works from the project list, not only inside a project. Gates green both repos (`lint:undef` + `build`). Browser-validated 2026-06-14. |
...
| 2026-05-21 | `specs/2026-05-21-pre-launch-roadmap.md` | ACTIVE (living launch spine — not a closeable deliverable) | Reconciled 2026-06-14. **Track A (estimator SQL push-down) Fases 0–8 all DONE** + R-validated (see DuckDB section). **Track C: X3 DONE; X1 DONE; X2 structural-half DONE; X4 and X5 implementation-complete with browser/runtime validation pending Franco** — see dedicated rows. **Remaining:** Track B module-validation V1 spatial / V2 calculate / V3 simulation / V4 plots (R-validation harnesses need Franco's R), X2 runtime smoke, X4 deployed-browser performance checks, X5 browser UX checks, and the launch gate per §"Done criteria". |
| 2026-05-22 | `plans/2026-05-22-fase-x1-pipeline-reliability.md` | DONE 2026-06-14 | Fase X1 of the pre-launch roadmap (Track C). Node-runnable harness `src/pipeline/__validation__/pipelineReliabilityValidation.mjs` (67 checks green): T5 registry↔runner sync (catches any registry type lacking a runner case), T1 per-step smoke over ALL 53 registry types + golden checks for the CLAUDE.md "key bugs" (lag/lead/diff cross-unit, winz creation-time bounds, normalize_cats), T4 non-destructive (deep-frozen rawData survives replay), T2/T3 composability + serialization round-trip determinism + order-sensitivity, T6 audit-trail completeness. **Surfaced + fixed:** (1) CLAUDE.md said "23/35 step types" — actually **53** (doc corrected, count now harness-derived); (2) `group_summarize` lacked the missing-`nn` fallback its sibling `group_transform` has → a malformed agg could create an "undefined"-keyed column; added `aggName()` defensive fallback (runner.js). Build + lint:undef green. Deferred per plan: live IndexedDB round-trip (browser-only; covered structurally by T4 serialization), async-worker numeric correctness (owned by exprGuard/stepValidator harnesses), undo/redo UI stack. Pre-merge Supabase feedback-collector gate in the plan still pending Franco. |
| 2026-05-22 | `plans/2026-05-22-fase-x2-replication-integrity.md` | DONE 2026-06-14 (structural; runtime smoke pending Franco's R/Py/Stata) | Fase X2 of the pre-launch roadmap (Track C). Node harness `src/services/export/__validation__/replicationIntegrityValidation.mjs` (269 checks green): T1 translator coverage (every registry type emits real R/Py/Stata, no fallback markers — the export-side analog of X1's sync check), T2 no-garbage (no `undefined`/`[object Object]` in emitted code), T3 three-language parity, T4 multi-subset bundle, T5 generator determinism. **Surfaced + fixed a MAJOR bug:** Python & Stata pipeline-step transpilers read a **dead `params`-nested step schema** (legacy field names like `params.from`/`params.entityCol`) while the app passes flat registry-shape steps → **every non-grid-edit data-prep step emitted `undefined` operands** in Python/Stata replication scripts (R was already migrated to flat). Grid-edit cases (added 2026-06-05) were flat = the migration tell. Fixes: migrated both Python & Stata `transpileStep` to the flat schema (~25 cases each, R as reference), added 11 missing R cases (`fill_na_grouped/trim_outliers/flag_outliers/extract_regex/factor_interactions/clean_strings/if_else/case_when/grouped_mutate/pivot_wider/balance_panel`) + 6 missing Py/Stata cases, removed duplicate `group_transform`/`factor_interactions` cases, fixed `buildStataVarlist` `rawW is not iterable` crash (no-`wVars` default). Also corrected: `std`/`trim_outliers`/`winz` now use step-creation-time fixed params (was recomputing). Build + lint:undef green. **Remaining (pending Franco's runtime):** actual smoke test — run exported scripts in R/statsmodels/Stata, assert coefficients match UI within 1e-6. **Side finding (out of scope, flagged):** `clean_strings` registry schema (`stripPunct/normSep/case`) ≠ runner fields (`ocrNoise/midWordSep`) — registry/runner mismatch worth a separate fix. |
...
| 2026-06-01 | `specs/2026-06-01-stat-sim-inference-deepening-design.md` | DONE — browser-validated 2026-06-14 | Spec A (data-level only): parametric tests (two-sample/paired/prop/corr/var-ratio), general bootstrap (perc/basic/BCa)+jackknife, generalized permutation, shared seeded RNG. Spec B (model-coef inference) deferred. Math + harness 81/81 green; UI Tasks 11–13 browser-validated 2026-06-14. |
...
| 2026-06-05 | `specs/2026-06-05-ai-execute-clean-commands-design.md` | DONE — browser-validated 2026-06-14 | Clean-module AI upgrade: NL → validated declarative pipeline steps (multi-column/multi-step), preview-then-confirm. Fixes geometry→lat/lon bug (single-column `ai_tr` can't emit 2 cols). Built: `appCapabilityMap.serializeAllowedSteps`, `stepValidator.validateAISteps` (+node harness 6/6), `NL_TO_PIPELINE_PROMPT`, `AIService.nlToPipeline`, `NLCommandBar.jsx` mounted in WranglingModule. Plan `plans/2026-06-05-ai-execute-clean-commands.md`. |
...
| 2026-06-03 | Callaway-Sant'Anna (2021) staggered DiD (item C3 of `specs/2026-06-01-modeling-poisson-twfe-gaps-design.md`) | IMPLEMENTATION COMPLETE — R validation pending Franco | Full rewrite per `plans/2026-06-27-callaway-santanna-rewrite.md` (10 tasks): drdid.js DR 2×2, staggeredDiD.js (enumerate+aggregate+Mammen+Wald), CallawayEngine.js orchestrator, EstimationResult wrap, EstimatorSidebar/ModelingTab wiring, didPlots.jsx, R/Stata/Python export, validation harness callawayRValidation.R → callawayBenchmarks.json. 2026-07-02: plot sizing/axis/table polish + PNG/LaTeX export added on top (commits `c1e8e213`, `58ff5490`) — browser-verified against a synthetic 2-cohort dataset. Only remaining gap: R validation pending Franco's run of callawayRValidation.R. |
