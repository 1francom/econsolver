...
| 2026-07-28 | `specs/2026-07-28-country-code-transform-design.md` | OPEN — spec approved by Franco, implementation plan not yet written | New Feature-tab pipeline step `country_code` (ISO2/ISO3/name conversion), analogous to R's `countrycode`. Reuses the `recode`/`normalize_cats` pattern: alias matching (~217-entry table, same universe as `worldBank.js#listCountries()`) happens once in the UI and is frozen into a literal `step.map` — runner.js and R/Python/Stata exports never see the alias table, only an exact dictionary lookup. `internal: true` in the registry (excluded from NL catalogue, same mechanism as `sp_*`/`patch` — the AI can't safely fabricate `map` without the dataset's real distinct values). Unmatched values → `null` + a mandatory preview showing the unmatched count before Apply. Scope v1: ISO2/ISO3/name only, no continent/region/numeric-ISO, no fuzzy/Levenshtein matching beyond the curated alias table.
