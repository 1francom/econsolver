...
| 2026-06-05 | `specs/2026-06-05-session-aware-coach-design.md` | DONE (browser-validation pending; sessionLog deferred) | Roadmap item 3: make Research Coach session-aware (thread `sessionSnapshot` into `researchCoach` — pipeline/pinned/subsets/inference/log) + app-aware (hybrid `appCapabilityMap.js`: curated tab shell + auto-derived steps from STEP_REGISTRY, cached system block). Plan `plans/2026-06-05-session-aware-coach.md` (8 tasks). Claude's parallel track — collision-free with Codex Batch 1 |
| 2026-06-05 | `specs/2026-06-05-excel-grid-editing-design.md` | OPEN | Roadmap item 2: Excel-grid editing in Data Viewer. 5 new steps (add_column, add_row, set_where, replace, str_splice) + structured filter bar as WHERE predicate. Non-destructive (emits pipeline steps). DataViewer (App.jsx) + DataStudio studio methods. Export translations sequenced after Codex Batch-1 export merge. Claude track |
