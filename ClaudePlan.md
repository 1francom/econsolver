...
| 2026-05-21 | `specs/2026-05-21-pre-launch-roadmap.md` | ACTIVE (living launch spine — not a closeable deliverable) | Reconciled 2026-06-10. **Track A (estimator SQL push-down) Fases 0–8 all DONE** + R-validated (see DuckDB section). **Track C: X3 (AI hardening) DONE** (faseX3 harness); **X1 (pipeline reliability) DONE 2026-06-14**; **X2 (replication integrity) structural-half DONE 2026-06-14** (runtime smoke pending Franco's R/Py/Stata) — see X1/X2 rows below. **Remaining:** Track B module-validation V1 spatial / V2 calculate / V3 simulation / V4 plots (R-validation harnesses — need Franco's R), and Track C X4 performance (Codex's lane, in progress) / X5 bug-bash+UX. **Fase X5 (bug bash + visual polish) = the upcoming feedback/visual pass.** Launch gate per §"Done criteria". |
