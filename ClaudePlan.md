...
| 2026-06-07 | `specs/2026-06-07-design-language-refresh-design.md` | OPEN | UI upgrade Workstream A (of A/B/C/D). Extend `theme.js`/`ThemeContext` to type/space/elev tokens; IBM Plex Sans+Mono pairing; Appearance config panel (gear in WorkspaceBar: font/density/plot-palette, keep day/night); incremental drift cleanup of hardcoded hex; author root `DESIGN.md`. No accent customization. B/C/D deferred |
| 2026-06-07 | `plans/2026-06-07-design-language-refresh.md` | OPEN | 10-task implementation plan for Workstream A spec: theme.js tokens + buildTokens (T1), node harness 9/9 (T2), ThemeContext prefs/persistence/fonts (T3), AppearancePanel (T4), WorkspaceBar gear (T5), plot palette from prefs (T6), drift cleanup batch1 high-traffic (T7) + batch2 spatial/sidebar (T8), root DESIGN.md (T9), validation matrix (T10) |
