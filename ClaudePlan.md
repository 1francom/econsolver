...
|------|-------------|--------|-------|
| 2026-09-17 | `plans/2026-09-17-real-data-validation.md` | OPEN — plan written, awaiting Franco's answers (project names, AJR data, package installs) | Real-data validation replacing the deleted simulated benchmarks: LMU Applied Econometrics PS1–PS7 + BA thesis. Litux numbers are recomputed headlessly from an exported project recipe (new *Export project* `.litux.json`), every full and per-section script is generated from the same specs and run in R/Stata/Python, all compared against the original reference script. Starts with PS5 (Waldinger Validation Tutorial 5, already built). |
