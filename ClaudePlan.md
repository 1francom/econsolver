...
| 2026-06-05 | `specs/2026-06-05-cloud-sync-e2ee-design.md` | OPEN | Roadmap #4 Phase 4a-1: opt-in, end-to-end-encrypted cross-device sync (local-by-default, "publish a project"). Separate sync passphrase (Argon2id/PBKDF2 → AES-GCM), zero-knowledge; recovery-key export; pull-on-login, debounced push, conflict-prompt; untrusted-pull guard reuses `exprGuard.assertSafeExpr`. Supabase `synced_projects` + Storage, per-user RLS. Sharing deferred (4a-2). Plan `plans/2026-06-05-cloud-sync-e2ee.md` (6 tasks) — delegated to Codex. Needs review + migration apply before merge |
