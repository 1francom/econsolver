...
| 2026-06-05 | `specs/2026-06-05-clean-explore-parity-vector-join-design.md` | OPEN | Batch 1 for Codex: join types (right/full/anti/semi), Combine tab (bind_cols/union/intersect/setdiff), distinct, group_transform, Vector Join (`vector_assign` 4 modes, seeded mulberry32). Authored by Claude, executed solo by Codex |
| 2026-06-05 | `specs/2026-06-05-session-aware-coach-design.md` | OPEN | Roadmap item 3: make Research Coach session-aware (thread `sessionSnapshot` into `researchCoach` — pipeline/pinned/subsets/inference/log) + app-aware (hybrid `appCapabilityMap.js`: curated tab shell + auto-derived steps from STEP_REGISTRY, cached system block). Claude's parallel track — collision-free with Codex Batch 1 |
| 2026-06-05 | `specs/2026-06-05-ai-execute-clean-commands-design.md` | OPEN | Clean-module AI upgrade: NL → validated declarative pipeline steps (multi-column/multi-step), preview-then-confirm. Fixes geometry→lat/lon bug (single-column `ai_tr` can't emit 2 cols). New `NL_TO_PIPELINE_PROMPT`, `nlToPipeline()`, `stepValidator.js`, `NLCommandBar.jsx` mounted by WranglingModule. Cleaning+Features scope only. Collision-free with Codex (emits existing step types) |
