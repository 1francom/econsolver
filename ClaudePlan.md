...
|------|-------------|--------|-------|
| 2026-07-22 | `specs/2026-07-22-explore-sql-fastpath-design.md` | IN PROGRESS — Phase 1 (Group & Summarize) DONE, Phases 1b/2-6 PENDING | Evidence Explorer's stats (Group & Summarize, Summary Table, Correlation, Histograms, Time Series, PlotBuilder scatter) computed off a JS row array that was either a 500-row DuckDB preview (wrong numbers, silently) or a full-table JS materialization (correct but doesn't scale to 900k+ rows, against the DuckDB performance strategy). Fix is pushing each tab's computation into SQL so the JS↔DuckDB boundary only ever crosses aggregated results. Phase 1 landed on `claude/plot-builder-binwidth-fix-32rx1h` (PR #118): `GroupSummarizeExplorer.doSummarize()` now runs a `GROUP BY` SQL query via `queryDuckDB()` when a `duckTable` is present and no QuickFilter is active. |
