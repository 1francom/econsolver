...
3. **PlotBuilder — remaining G-track** — G13 (multi-model overlay). ~~G3~~ (smooth, boxplot, errorbar, ribbon), ~~G5~~ (stack/jitter), ~~G6~~ (hline/vline), ~~G7~~ (palette presets), ~~G9~~ (SVG/PNG export), ~~G10~~ (estimator templates: resid vs fitted, resid dist, actual vs fitted, Y vs X₁), ~~G12~~ (PlotBuilder in ModelingTab with result-augmented rows __resid__/__yhat__) done. G1+G2+G4+G8+G11 also done.
