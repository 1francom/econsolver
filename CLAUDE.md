...
2a. ~~**DuckDB OLS suff-stats SQL push-down (Fase 0+1)**~~ — large-n OLS never materializes data in JS. `duckdbDispatch.shouldUseSQLPath` routes to SQL when n≥50k, k≤100, OLS, classical/HC0-3, no weights. `duckdbOLS.buildOLSSuffStats` builds X'X/X'Y/Y'Y/sumY in SQL; cached LRU by `(table, y, sorted xColsExpanded)` in `suffStatsCache`. Fase 1 extensions: `duckdbFactors.expandFactors` (CASE WHEN dummies, first level dropped), `duckdbRobustSE.computeHCMeat`/`computeHCMeatWithLeverage` (HC0/HC1 + leverage-based HC2/HC3 with β/Ainv as prepared params + aggCount sanity throw), `duckdbResiduals.sampleResiduals` (lazy 5k-row thunk via `USING SAMPLE`), `duckdbDiagnostics` (BP/DW/JB via CTE). `runOLSFromSuffStats({meat, hcType})` does V=Ainv·meat·Ainv; HC1 scales n/(n-k); hcType uppercased at boundary. ModelingTab `estimate()` normalizes lowercase UI seType `hc1→HC1` before dispatch. Validation: 52/52 structural + R `sandwich::vcovHC` golden values in `__validation__/fase1Benchmarks.json` (window.__validation.fase1).
