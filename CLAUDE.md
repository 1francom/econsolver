...
~~**DuckDB Fase 5 — DiD + Event Study SQL fast paths (2026-05-21)**~~ — ✓ complete. `duckdbDiDSynthetic` emits DiD interaction and Event Study horizon/bin SQL payloads. Large-n `DiD` reuses OLS suff stats; `TWFE` and `EventStudy` reuse TWFE within suff stats and preserve ATT/event-coefficient/pre-trend result contracts. Validation fixtures: base-R `fase5RValidation.R`, generated `fase5Benchmarks.json`, and `window.__validation.fase5`.
**DuckDB Fase 6 — IRLS SQL fast paths (started 2026-05-21)** — Logit/Probit/Poisson FE now have an initial DuckDB IRLS branch with a higher 200k dispatch threshold, HC0/HC1 score-residual meat, and direct base-R/browser validation fixtures (`fase6RValidation.R`, `window.__validation.fase6`). Keep it in progress until the browser validation cells are green.
**DuckDB Fase 7 — RDD SQL fast paths (implementation complete 2026-05-21)** — Large-n live `RDD` and `FuzzyRDD` ids route through triangular-kernel WLS suff stats with SQL IK bandwidth moments. `duckdbWLS`/`duckdbWLSRobustSE` accept kernel `weightSQL`; `RDDSuffStatsEngine` returns sharp discontinuities and fuzzy Wald/delta-method results, and McCrary density bins are aggregated in SQL before the small local fit. Base-R fixtures and `window.__validation.fase7` are added; browser validation is left pending.
**DuckDB Fase 8 — robust-SE backfill (implementation complete 2026-05-21)** — WLS adds HC2/HC3, clustered, two-way clustered, and HAC SQL score meats; 2SLS adds the fitted-design variants; LIML now accepts HC0/HC1, clustered, and HAC SQL meat. Self-contained R fixtures and `window.__validation.fase8` are added; browser validation remains pending.
