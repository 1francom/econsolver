...
5a. **Polynomial RDD validation (PENDING)** — `runSharpRDD`/`runFuzzyRDD`/`runSpatialRDD` now accept `polyOrder` (1/2/3). Needs R validation: compare p=2 JS coefficients against `rdrobust(y, x, c, p=2)` — target 6dp coef, 4dp SE. SQL fast path already blocked for p>1 (throws to JS fallback). Validation fixtures to add: `polyRDDRValidation.R`, `polyRDDBenchmarks.json`, `window.__validation.polyRDD`.
